...
export default function EditProductPage() {
  ...
  const [info, setInfo] = useState({ name: "", short_description: "", description: "", category_id: "", hsn_code: "", gst_rate: "18", price_type: "fixed", is_featured: false,is_active:true });
...
const loadProduct = async () => {
  try {
    ...
    setInfo({
      ...
      gst_rate: String(res.data.gst_rate || "18"),
      price_type: res.data.price_type || "fixed",
    ...
    if (res.data.variants?.length) {
      setVariants(
        res.data.variants.map((v: any) => ({
          ...
          price: String(v.retail_price || ""),
          trade_price: String(v.trade_price || ""),
  ...
  return (
    <div style={{ padding: 32, maxWidth: 1000 }}>
      <PageHeader title="Update Product" subtitle="Update product "
