...
interface Variant {
  ...
  sku: string;
  retail_price: number;
  product: { name: string; gst_rate: number };
}
...
function calcLine(line: ReturnLine): ReturnLine {
  ...
  const tax     = r2(taxable * line.gst_rate / 100);
  return { ...line, taxable_amount: taxable, tax_amount: tax, line_total: r2(taxable + tax) };
}

const emptyLine = (): ReturnLine => ({
  variant_id: 0, product_name: "", sku: "", unit_price: 0, quantity: 1, gst_rate: 18,
...
export default function CreateSalesReturnPage() {
  ...
  // manual mode — customer lookup
  const [customerId, setCustomerId] = useState("");
  const [customerSearch, setCustomerSearch] = useState("");
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [showCustDrop, setShowCustDrop] = useState(false);

  // shared line items (works for both modes)
  ...
  // ── lookup invoice, prefill lines from its items ──
  const lookupInvoice = async () => {
    ...
    try {
      ...
      const newLines: ReturnLine[] = (inv.items || []).map((it: any) =>
        calcLine({
          variant_id: it.variant_id ,
                    product_name: it.product_name,
          sku: it.sku || "",
          unit_price: it.unit_price,
          quantity: it.quantity, // staff chooses how many of the invoiced qty are returned
  ...
  const switchMode = (m: "invoice" | "manual") => {
    setMode(m);
    setError("");
    setLines([emptyLine()]);
    setVariantSearch([""]);
  ...
  const handleSave = async () => {
    ...
    if (mode === "invoice" && !invoiceNumber) { setError("Look up an invoice first"); return; }
    if (mode === "manual" && !customerId) { setError("Select a customer"); return; }
    if (validLines.length === 0) { setError("Add at least one item with a quantity greater than 0"); return; }

    setSaving(true);
    try {
      const payload: any = {
        invoice_number: mode === "invoice" ? invoiceNumber : null,
    ...
    setCreated(true); // lock the form
    alert(`Return ${r.data.return_number} created successfully`);
    router.push(`/admin/accounting/invoices/return`);
  } catch (e: any) {
    setError(parseApiError(e));
    setSaving(false); // only reset on failure, so they can retry
  }
};
  return (
    <div style={{ padding: 28, maxWidth: 1200, margin: "0 auto" }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: "#0f172a", margin: 0 }}>New Sales Return</h1>
        <p style={{ color: "#64748b", fontSize: 13, margin: "4px 0 0" }}>Return items from an invoice, or enter manually</p>
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {mode === "invoice" ? (
              <div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    style={{ ...inputStyle, borderColor: invoiceNumber ? "#0284c7" : "#e2e8f0" }}
                    placeholder="Enter invoice number…"
                    value={invoiceNumberInput}
                    onChange={e => setInvoiceNumberInput(e.target.value)}
                    onKeyDown={e => e.key === "Enter" && lookupInvoice()}
                  />
                  <button onClick={lookupInvoice} disabled={fetching}
                ...
                {invoiceNumber && (
                  <p style={{ fontSize: 13, color: "#16a34a", margin: "10px 0 0" }}>
                    ✓ Invoice #{invoiceNumber} — {billingName}
                  </p>
                )}
              </div>
            ) : (
              <div style={{ position: "relative" }}>
                ...
                <p style={{ fontSize: 12, color: "#94a3b8", margin: "10px 0 0" }}>
                  Search and add items directly below — no invoice required.
                </p>
              </div>
            )}
          </div>
          ...
          <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, overflow: "hidden" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "2fr 80px 110px 70px 100px 36px", gap: 8, padding: "10px 16px", borderBottom: "1px solid #f1f5f9" }}>
              {["Product / Variant", "Qty", "Unit Price", "GST %", "Total", ""].map(h => (
            ...
            {lines.map((line, idx) => (
              <div key={idx} style={{ padding: "10px 16px", borderBottom: "1px solid #f8fafc" }}>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 80px 110px 70px 100px 36px", gap: 8, alignItems: "center" }}>
                  ...
                  <input type="number" min="0" style={inputStyle} value={line.gst_rate}
                    disabled={mode === "invoice"}
  ...
  {created ? "✓ Created" : saving ? "Creating…" : "💾 Create Return"}
          ...
          <div style={{ padding: "12px 18px", background: "#eff6ff", borderTop: "1px solid #bfdbfe", fontSize: 12, color: "#1e40af" }}>
            ℹ️ Approving a return issues a credit note and restores stock.
          </div>
        </div>
      </div>
    </div>
